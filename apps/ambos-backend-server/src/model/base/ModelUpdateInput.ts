/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { EnumModelGender } from "./EnumModelGender";
import {
  IsEnum,
  IsOptional,
  IsString,
  MaxLength,
  ValidateNested,
} from "class-validator";
import { OrderUpdateManyWithoutModelsInput } from "./OrderUpdateManyWithoutModelsInput";
import { Type } from "class-transformer";
import { ProductUpdateManyWithoutModelsInput } from "./ProductUpdateManyWithoutModelsInput";

@InputType()
class ModelUpdateInput {
  @ApiProperty({
    required: false,
    enum: EnumModelGender,
  })
  @IsEnum(EnumModelGender)
  @IsOptional()
  @Field(() => EnumModelGender, {
    nullable: true,
  })
  gender?: "Option1" | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => OrderUpdateManyWithoutModelsInput,
  })
  @ValidateNested()
  @Type(() => OrderUpdateManyWithoutModelsInput)
  @IsOptional()
  @Field(() => OrderUpdateManyWithoutModelsInput, {
    nullable: true,
  })
  orders?: OrderUpdateManyWithoutModelsInput;

  @ApiProperty({
    required: false,
    type: () => ProductUpdateManyWithoutModelsInput,
  })
  @ValidateNested()
  @Type(() => ProductUpdateManyWithoutModelsInput)
  @IsOptional()
  @Field(() => ProductUpdateManyWithoutModelsInput, {
    nullable: true,
  })
  products?: ProductUpdateManyWithoutModelsInput;
}

export { ModelUpdateInput as ModelUpdateInput };
